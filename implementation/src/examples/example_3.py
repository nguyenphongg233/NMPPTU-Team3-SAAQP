...
def run_gda(x0, max_iters, f, f_dx, n, sigma, lamda, K, eps, verbose=True):
...
def run_gd(x0, max_iters, f, f_dx, n, lamda, eps, verbose=True):
...
def run_experiment_for_n(local_n, seed=None):
    ...
    t0 = time.time()
    trajectory_gda, values_gda = run_gda(x0, max_iters, f, f_dx, n, sigma, lamda_gda, K, eps, verbose=True)
    t1 = time.time()
    gda_time = t1 - t0
    ...
    x0 = find_min(x0, n)
    t0 = time.time()
    trajectory_gd, values_gd = run_gd(x0, max_iters, f, f_dx, n, lamda_gd, eps, verbose=True)
    t1 = time.time()
    gd_time = t1 - t0
...
if __name__ == '__main__':
    ...
    run_sweep([500,600,700,800,900,1000])