...
def main():
    n = 100
    max_iters_gda = 1000
    max_iters_rnn = 10*max_iters_gda
    num_init_points = 1
    ...
    for i in range(num_init_points):
        ...
        res_gda, val_gda = gda_instance.gda(x0, max_iters_gda, f, f_dx, n)
        ...
        res_rnn_hist, xt_rnn = rnn_instance.rnn(x1, max_iters_rnn, f, f_dx, g_i, derivative_g_i)
        end_time_rnn = time.time()
        time_rnn = end_time_rnn - start_time_rnn
        
        tmp_rnn = np.array(res_rnn_hist)[:,:]
        final_val_rnn = -np.log(-f(xt_rnn))

        sol_rnn_all.append(tmp_rnn)
        val_rnn_all.append(final_val_rnn)
        print("RNN-Agorithms : Initial point:",x1,"Final point:",xt_rnn,"Final value:",final_val_rnn)
        print(f"RNN Execution Time: {time_rnn:.6f} seconds\n")
        
        count += 1

        #plot_x(sol_gda_all, sol_rnn_all, count, max_iters_gda)
